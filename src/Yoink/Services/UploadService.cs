...
public static class UploadService
{
    ...
    public static string GetName(UploadDestination dest) => dest switch
    {
        ...
        UploadDestination.Streamable => "Streamable",
    ...
    public static string GetHistoryLogoPath(string? provider)
    {
        return (provider ?? string.Empty).ToLowerInvariant() switch
        {
            ...
            "streamable" => "Assets/streamable_sq.png",
    ...
    public static string GetUploadsLogoPath(UploadDestination dest) => dest switch
    {
        ...
        UploadDestination.Streamable => "Assets/streamable_sq.png",
    ...
    public static long GetMaxSize(UploadDestination dest, string filePath)
    {
        ...
        return dest switch
        {
            ...
            UploadDestination.Streamable => 250L * 1024 * 1024,
        ...
        string filePath, UploadDestination dest, UploadSettings settings)
    {
        try
        {
            ...
            return dest switch
            {
                ...
                UploadDestination.Streamable => await UploadStreamable(filePath, settings),
    ...
    private static async Task<UploadResult> UploadImgur(string filePath, UploadSettings s)
    {
        ...
        return new UploadResult
        {
            Error = node?["data"]?["error"]?.GetValue<string>() ?? $"Imgur error: {resp.StatusCode}"
        };
    }
    ...
    private static async Task<UploadResult> UploadImgBB(string filePath, UploadSettings s)
    {
        ...
        return new UploadResult
        {
            Error = node?["error"]?["message"]?.GetValue<string>() ?? $"ImgBB error: {resp.StatusCode}"
        };
    }
    ...
    private static async Task<UploadResult> UploadGyazo(string filePath, UploadSettings s)
    {
        ...
        return new UploadResult { Error = node?["message"]?.GetValue<string>() ?? $"Gyazo error: {resp.StatusCode}" };
    }
    ...
    private static async Task<UploadResult> UploadFileIo(string filePath)
    {
        ...
        return new UploadResult { Error = node?["message"]?.GetValue<string>() ?? $"file.io error: {resp.StatusCode}" };
    }
    ...
    private static async Task<UploadResult> UploadDropbox(string filePath, UploadSettings s)
    {
        ...
        if (!shareResp.IsSuccessStatusCode && shareBody.Contains("shared_link_already_exists"))
        {
            ...
            var listNode = JsonNode.Parse(listBody);
        ...
        {
            var shareNode = JsonNode.Parse(shareBody);
    ...
    private static async Task<UploadResult> UploadStreamable(string filePath, UploadSettings s)
    {
        if (string.IsNullOrWhiteSpace(s.StreamableEmail) || string.IsNullOrWhiteSpace(s.StreamablePassword))
            return new UploadResult { Error = "Streamable email/password not configured" };
        ...
        using var req = new HttpRequestMessage(HttpMethod.Post, "https://api.streamable.com/upload");
        var auth = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{s.StreamableEmail}:{s.StreamablePassword}"));
        req.Headers.Authorization = new AuthenticationHeaderValue("Basic", auth);
        req.Content = content;
        var resp = await Http.SendAsync(req);
        var body = await resp.Content.ReadAsStringAsync();
        if (!resp.IsSuccessStatusCode)
            return new UploadResult { Error = $"Streamable upload failed: {resp.StatusCode}" };

        var node = JsonNode.Parse(body);
        var shortcode = node?["shortcode"]?.GetValue<string>();
        return !string.IsNullOrWhiteSpace(shortcode)
            ? new UploadResult { Success = true, Url = "https://streamable.com/" + shortcode }
            : new UploadResult { Error = "Streamable returned no shortcode" };
    }
...
public sealed class UploadSettings
{
    ...
    // Streamable
    public string StreamableEmail { get; set; } = "";
